...
export function useMonthRevenue() {
  ...
  useEffect(() => {
    if (cache && Date.now() - cache.ts < TTL) { setRevenue(cache.revenue); setTs(cache.ts); setLoading(false); return; }
    let cancelled = false;
    ...
    const t = setTimeout(async () => {
      try {
        ...
        if (!cancelled) { setRevenue(j.revenue); setTs(cache.ts); setLoading(false); }
      } catch {
